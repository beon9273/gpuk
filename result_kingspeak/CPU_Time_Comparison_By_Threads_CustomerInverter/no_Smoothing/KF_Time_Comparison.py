...
def time_comparison_plotting(time_collection):
    ...
    threads = [2**i for i in range(len(time_collection)-2)]
    threads.append(28)
    threads.append(28*2)
    ...
    plt.plot([i for i in range(len(time_collection))], time_collection, marker='o', color='r')
    ...
    plt.title('Time Consumpting CPU along numThreads with CustomerInverter-no Smoothing')
    plt.savefig('KF_Time_Fitting_Comparison_CPU_CustomerInverter.pdf')
    plt.close()
